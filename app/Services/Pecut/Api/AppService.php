...
namespace App\Services\Pecut\Api;
...
class AppService
{
    ...
    public function getPaginatedApps(Request $request): array
    {
        try {
    ...
    public function getUrusanOptions(): array
    {
        $ids = AppLink::query()
            ->where('is_active', true)
            ->whereDoesntHave('children')
    ...
    public function getOpdOptions(): array
    {
    ...
    public function getPopularApps(Request $request): array
    {
        try {
            $search = trim((string) $request->query('search', ''));
            $categoryId = $request->query('category_id');
            ...
            $query->orderByDesc('visit_count')->limit($limit);
            $apps = $query->get();

            AppLinkResource::withoutWrapping();
            $items = AppLinkResource::collection($apps);
