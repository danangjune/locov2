...
function TextInput({ label, value, onChange, error, placeholder, type = "text" }) {
    return (
        <div>
...
export default function AppForm({ mode = "create", app = null, parent = null, options = {}, parentOptions = [], defaults = {} }) {
    ...
    return (
        <form onSubmit={submit} className="space-y-6">
            <div className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm shadow-slate-200">
                <div className="flex flex-col gap-4 border-b border-slate-100 pb-6 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                        ...
                        <h2 className="mt-2 text-2xl font-black text-slate-950">
                            {isEdit ? "Edit Data Aplikasi" : "Tambah Data Aplikasi"}
                        </h2>
                        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
                            Lengkapi informasi utama aplikasi. Jika membuat child, pilih parent sesuai struktur OPD atau unit layanan.
                        </p>
                    </div>
                    <Link
                        href="/admin/apps"
                ...
                {parent && !isEdit && (
                    <div className="mt-6 rounded-2xl border border-sky-100 bg-sky-50 p-4">
                        <p className="text-xs font-black uppercase tracking-widest text-sky-700">Child dari</p>
                        <p className="mt-1 text-base font-black text-slate-900">{parent.name}</p>
                        <p className="mt-1 text-sm font-semibold text-slate-500">{parent.category?.title || "-"} • {parent.urusan?.title || "-"}</p>
                    </div>
                )}
                ...
                <div className="mt-6 grid gap-5 lg:grid-cols-2">
                    <SearchableSelect
                        ...
                        isOptionDisabled={(item) => isEdit && Number(item.id) === Number(app?.id)}
                        renderSelected={(item) => (
                            <span className="block min-w-0">
                                <span className="block truncate font-black text-slate-700">{item.name}</span>
                                {item.urusan ? (
                    ...
                    <SearchableSelect
                        ...
                        onChange={(value) => setData("app_from_id", value || "")}
            ...
            <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
                <div className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm shadow-slate-200">
                    <p className="text-sm font-black uppercase tracking-widest text-sky-600">Pengaturan</p>
                    <div className="mt-5 grid gap-4 sm:grid-cols-2">
                        <label className="flex cursor-pointer items-center justify-between gap-4 rounded-2xl border border-slate-100 bg-slate-50 p-4">
                            <div>
                                <p className="font-black text-slate-900">Aktif</p>
                                <p className="mt-1 text-sm text-slate-500">Tampilkan aplikasi di portal.</p>
                            </div>
                            <input
                                type="checkbox"
                        ...
                        <label className="flex cursor-pointer items-center justify-between gap-4 rounded-2xl border border-slate-100 bg-slate-50 p-4">
                            <div>
                                ...
                                <p className="mt-1 text-sm text-slate-500">Tandai aplikasi terintegrasi SSO.</p>
                            </div>
                            <input
                                type="checkbox"
                                checked={Boolean(data.is_sso)}
                                onChange={(event) => setData("is_sso", event.target.checked)}
                                className="h-5 w-5 rounded border-slate-300 text-sky-600 focus:ring-sky-500"
                            />
                        </label>
                    </div>
                ...
                <div className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm shadow-slate-200">
                    <p className="text-sm font-black uppercase tracking-widest text-sky-600">Logo / Gambar</p>
                    <div className="mt-5 overflow-hidden rounded-2xl border border-slate-100 bg-slate-50">
                        {preview ? (
                            <img src={preview} alt="Preview" className="h-48 w-full object-contain p-4" />
                        ) : (
            ...
            <div className="sticky bottom-4 z-20 flex justify-end">
                ...
                >
                    <Save className="h-4 w-4" /> {processing ? "Menyimpan..." : "Simpan Aplikasi"}
                </button>
            </div>
        </form>
    );
